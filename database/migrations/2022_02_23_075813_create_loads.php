...
class CreateLoads extends Migration
{
    ...
    public function up()
    {
        Schema::create('loads', function (Blueprint $table) {
            ...
            $table->string('trailer_license_plate', 25)->nullable()->default('NULL')->comment('Matrícula del remolque del método de transporte');
            $table->foreignId('carrier_id')->nullable()->comment('Indica la empresa de transporte que realiza la carga')->constrained('carriers');
            $table->foreignId('exit_transport_id')->nullable()->comment('Indica el medio de transporte de salida de los vehículos')->constrained('transports');
            $table->foreignId('compound_id')->nullable()->comment('Indica la campa donde se realiza la carga')->constrained('compounds');
